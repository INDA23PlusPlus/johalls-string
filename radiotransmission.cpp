...
// constexpr ull MOD = 103895000908142711;
constexpr ull MOD = 1e9 + 7;
constexpr ull PRIME = 1097;
constexpr ull N = 1e6;
...
int main() {
    ...
    rep(i, 0, sz(inp)) prefix_hash[i + 1] = (prefix_hash[i] * PRIME % MOD + (inp[i] - 'a' + 1)) % MOD;
    auto get_hash = [&](auto left, auto right) -> ull {
        return (prefix_hash[right] - prefix_hash[left] * pows[right - left] % MOD + MOD) % MOD;
    };

    rep(len, 1, n + 1) {
        ull corr_hash = get_hash(0, len);
        ull corr_hash_whole = 0;
        ull plen = ipow(PRIME, len);
        ull num = n / len;
        ull hash_until_remainder = (ipow(plen, num) + MOD - 1) % MOD * inv(plen + MOD - 1) % MOD * corr_hash % MOD;
        ull hash_remainder = get_hash(num * len, num * len + n % len);

        if (hash_until_remainder == get_hash(0, num * len) && hash_remainder == get_hash(0, n % len)) {
            println(len);
            return 0;
        }
    }
