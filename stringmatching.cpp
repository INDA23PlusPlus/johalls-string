...
int main() {
    ...
    while (readln(needle) && readln(haystack)) {
        ull needle_hash = 0;
        for (auto i: needle)
            needle_hash *= 31, needle_hash += i;
        ull haystack_hash = 0;
        for (auto i: string_view(haystack).substr(0, sz(needle)))
            haystack_hash *= 31, haystack_hash += i;
        if (haystack_hash == needle_hash) print(0, "");

        ull big = 1;
        rep(i, 1, sz(needle)) big *= 31;
        rep(i, 0, sz(haystack) - sz(needle)) {
            haystack_hash -= haystack[i] * big;
            haystack_hash *= 31;
            haystack_hash += haystack[i + sz(needle)];
            if (haystack_hash == needle_hash) print(i + 1, "");
        }
        print("\n");
    }
}
