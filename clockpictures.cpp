...
int main() {
    ...
    ull hash_needle = diff(a.back(), a.front());
    rep(i, 1, n) hash_needle = hash_needle * 31 + diff(a[i - 1], a[i]);

    vi haystack(n);
    haystack[0] = diff(b.back(), b.front());
    ull hash_haystack = haystack[0];

    ull big = 1;
    rep(i, 1, n) {
        haystack[i] = diff(b[i - 1], b[i]);
        big *= 31;
        hash_haystack = hash_haystack * 31 + haystack[i];
    }
    rep(i, 0, n) {
        hash_haystack -= big * haystack[i];
        hash_haystack *= 31;
        hash_haystack += haystack[i];
