...
int main() {
    ...
    read(haystack, needle);
    for (auto &i: haystack) i -= 'a';
    for (auto &i: needle) i -= 'a';

    alignas(32) uint32_t needle_order[32]{};
    alignas(32) uint32_t first_pos[32]{};
    alignas(32) uint32_t needle_hashes[32]{};
    uint32_t needle_num_unique = 1;
    rep(id, 0, sz(needle)) {
        char i = needle[id];
        if (!needle_order[i]) {
            first_pos[i] = id;
            needle_order[i] = needle_num_unique++;
        }
        for (auto &j: needle_hashes) j *= PRIME;
        needle_hashes[i]++;
    }

    alignas(32) uint32_t haystack_hashes[32]{};
    rep(id, 0, sz(needle)) {
        char i = haystack[id];
        for (auto &j: haystack_hashes) j *= PRIME;
        haystack_hashes[i]++;
    }

    uint32_t big = 1;
    rep(i, 1, sz(needle)) big *= PRIME;

    uint32_t candidate = 0, num_candidates = 0;

    rep(start_point, 0, sz(haystack) - sz(needle) + 1) {
        bool works = true;
        for (auto i: first_pos) {
            if (needle_hashes[needle[i]] != haystack_hashes[haystack[start_point + i]]) {
                works = false;
                break;
            }
        }

        // we dont have time to double check our candidates 
        // (unless we employ some KMP type reasoning for jumping ahead but it turns out we dont have to)
        if (works) candidate = start_point, num_candidates++;

        if (start_point + sz(needle) < sz(haystack)) {
            haystack_hashes[haystack[start_point]] -= big;
            for (auto &j: haystack_hashes) j *= PRIME;
            haystack_hashes[haystack[start_point + sz(needle)]]++;
        }
    }

    if (num_candidates == 1) {
        auto res = haystack.substr(candidate, sz(needle));
        for (auto &i: res) i += 'a';
        println(res);
    } else {
